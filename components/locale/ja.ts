const FIXED_JAPANESE_WORD = {
  PROGRAM_SRC: "https://www.youtube.com/embed/aAqwxCyBdRE?si=JMku9plZsV1HDoMQ",
  PROJECT: "プロジェクト",
  PROJECT1: "https://www.youtube.com/embed/3owMhK2Bkwc?si=H744KBGkzijQb9gc",
  PROJECT1_DETAILS: "コトゥー住民のパキャン村には現在、電気と水が供給されている。人々は自分たちの習慣に従って作物を育てたり家畜を育てたりします。各世帯には 500 平方メートルを超える庭の面積がありますが、ほとんど空き地のままです。現在植物は多くなく、家畜もあまり飼っていません。ここに住むこれらのグループには、園芸と牧畜の技術と技能が理解していません。\nこのプロジェクトは、彼らが家畜を育て、家庭菜園を作るためのスキルとともに実践的な知識を指導することを目的としています。その結果、私たちはコミュニティの生活水準と社会的改善を効果的に向上させることを期待しています。これが、この村のよりクリーンな環境と人々の健康と安全の向上に貢献すると私たちは信じています。",
  PROJECT2: "https://www.youtube.com/embed/1OSXd4xcsyI?si=MfSLA90_aR5SXdAH",
  PROJECT2_DETAILS: "有機肥料は作物の品質と収量を決める重要な要素です。このプロジェクトは、村内の家畜や家禽の排泄物、農業廃棄物、家庭廃棄物を肥料源として再利用し、人々の庭や農業の経済モデルに供給することを目的としています。\nその結果、人々は廃棄物から有機肥料を作るプロセスを理解しました。人々は自分の庭に高品質の有機肥料を低コストで入手できます。また、村内の緑豊かな環境づくりにも貢献します。",
  ACHIEVEMENT: "成果",
  BLOG: "ブログ",
  INCOMING_PROJECTS: "入ってくるプロジェクト",
  CONTRIBUTORS: "コントリビューター",
  CONTACT: "お問い合わせ",
  SPONSORS: "スポンサー",
  QACONTACT: "Q&A & お問い合わせ",
  PROGRESS: "進捗",
  PROJECT_NAME: "ナムザン地区、カディコミューン、パオン村、パチャンの新しい住宅クラスターの開発",
  LOCATION: "クアンナム省ナムザン地区Ca-Dyコミューン",
  BUTTON_BE_THE_CONTRIBUTOR_OR_SPONSOR:
    "コントリビューターまたはスポンサーになる",
  DETAILS: "詳細",
  PLAN: "プラン",
  DESCRIPTION: "自然と密接に暮らすパチャンの住宅群は経済的困難に直面している。 Smile Eye Charity は、住民に経済的および生活スキルを提供し、新たな機会を創出し、生活の質を向上させるための支援を呼びかけています。 参加して愛を分かち合い、彼らの成長と繁栄を助けましょう。",
  DESCRIPTION_DETAILS: "説明の詳細",
};

export default FIXED_JAPANESE_WORD;
